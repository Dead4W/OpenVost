...
function unbindEpisode(id) {
    var actualCode = "document.getElementById('" + id + "').onclick = null;";
...
function open_vost(id,snum,autoplay,startPlayTime) {
    ...
    if( player ) {
        player.Set("poster","http://media.aniland.org/img/" + id + ".jpg");
        if( autoplay ) {
            player.Play("http://tk.aniland.org/" + id + ".mp4|http://fast.aniland.org/" + id + ".mp4|http://mp4.aniland.org/" + id + ".mp4");
        } else {
            player.Set("file","http://tk.aniland.org/" + id + ".mp4|http://fast.aniland.org/" + id + ".mp4|http://mp4.aniland.org/" + id + ".mp4");
            player.Stop();
        }
    } else {
        player = new Uppod({
    ...
    if(startPlayTime && startPlayTime > 5) {
...
$elems.each(function() {
    ...
    if (!--count) {
        setInterval(function() {
            if( $("#kinoplayer").css('display') == "none" && player.getStatus() === 1 && player.Duration() > 0 && player.Duration() - player.currentTime() >= 10 && Math.floor($.cookie("time" + window.location.pathname)) != player.currentTime() ) {
