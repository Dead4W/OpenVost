...
function checkBadFindServers(id) {
    var currentPlayer = activePlayer === "player2" ? player : kinoPlayer;
    if( currentPlayer.Get('episode_id') == id && openvost_ajax_servers.length ) {
		for( var i=0;i<openvost_ajax_servers.length;i++ ) {
			let server = openvost_ajax_servers[i];
			if( server.readyState !== 0 || server.status !== 0 || server.statusText !== "error" ) {
				return;
			}
		}
		console.log('[OpenVost]: fail auto find server');
		addVideoUrl(id,"http://new.aniland.org/" + id + ".mp4");
		addVideoUrl(id,"http://old.aniland.org/" + id + ".mp4");
		addVideoUrl(id,"http://fast.aniland.org/" + id + ".mp4");
		addVideoUrl(id,"http://mp4.aniland.org/" + id + ".mp4");
    }
}
